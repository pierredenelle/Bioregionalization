...
```{r run_OSLOM}
sp_proj <- read.table("OSLOM2/vignette.txt", header = TRUE)

# Try of run_oslom
run_oslom(sp_proj, n_runs = 10, t_param = 0.1, cp_param = 0.5,
          saving_directory = "D:/PIERRE_DENELLE/CarHab/test_bioregionalization/")
```
...
res <- readRDS("../../test_bioregionalization/tp.rds")
...
top10 <- z_scores %>%
  ...
  select(sp, bioregion, zscore, rank) %>%
