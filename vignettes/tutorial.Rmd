...
sp_mat <- contingency(medit[[1]], "id", "sp", ab = NULL, binary = TRUE)
...
```{r projection}
sp_proj <- project_network(sp_mat, similarity = "jaccard")
sp_proj <- sp_proj[, c("id1", "id2", "jaccard")]
...
medit[[1]]$id <- as.character(medit[[1]]$id)
tmp <- left_join(medit[[1]], oslom_vignette, by = c("id" = "site"))
z_scores <- zscore(tmp, sp_col = "sp", site_col = "id",
