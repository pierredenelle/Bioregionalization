...
interact(dat = zscores, plot = FALSE, output_format = "matrix")

```

```{r z_scores}
# tmp <- left_join(sp_df, oslom_vignette, by = "site")
tmp <- left_join(sp_df[which(sp_df$pa == 1), ], oslom_vignette, by = "site")
...
link_cz <- sp_df[which(sp_df$pa > 0), c("site", "sp")] %>%
...
po <- vmeasure_calc(x = bioregions_sf, x_name = oslom, y = bioregions_sf, y_name = ward)
...
plot_grid(
  ...
  # Plot of combined groups of pixels
  ggplot(all_bioregions, aes(x, y)) +
    geom_tile(aes(fill = as.factor(combined), color = as.factor(combined)),
              alpha = 0.8, width = 1, height = 1) +
    scale_color_brewer("Bioregions", palette = "Paired") +
    scale_fill_brewer("Bioregions", palette = "Paired", na.value = "grey") +
    coord_equal() +
    labs(title = "Combined groups of pixels") +
    theme(panel.background = element_rect(fill = "transparent", colour = NA)),
